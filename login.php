    ...
    if ($result) {
        if (mysqli_num_rows($result) === 1) {
            ...
            if ($row['password'] === $password) {
                ...
                // Fetch credit history from total_transactions table
                $credit_history_query = "SELECT credit_history FROM total_transactions WHERE user_email='{$email}' ORDER BY id DESC LIMIT 1";

                $credit_history_result = mysqli_query($conn, $credit_history_query);

                if ($credit_history_result) {
                    $credit_history_data = mysqli_fetch_assoc($credit_history_result);
                    $credit_history = $credit_history_data['credit_history'];
                    $credit_history_msg = "Credit History: " . $credit_history;
                } else {
                    $msg = "<div class='alert alert-danger'>Error fetching credit history: " . mysqli_error($conn) . "</div>";
                }
            } else {
                $msg = "<div class='alert alert-danger'>Password incorrect.</div>";
            }
        } else {
            $msg = "<div class='alert alert-danger'>User not registered.</div>";
        }
    } else {
...
<header class="header sticky">
  ...
  <img src="2914527.jpg" alt="" class="img1" />
      ...
      <?php if (!empty($credit_history_msg)) { ?>
                <div class="center mt-20"><?php echo $credit_history_msg; ?></div>
            <?php } ?>
		</div>
	</div>
